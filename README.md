...
如果设计者从一开始就用数据结构来定义业务对象、接口边界和流转关系，那么系统的边界会变得更清晰。数据结构不是实现细节，而是协作语言。整体数据定义是贯穿数据库、服务、接口和界面的共同契约，其中每个具体结构仍需声明自己的所有者和有效边界。
...
- 每个参与数据流的数据结构必须明确所有者、产生点、有效范围、转换或终止点，以及必要的变化和留存规则。
...
结构驱动开发的目标，是把这些摩擦前置到设计阶段解决。先定义对象、字段、约束、所有者、来源、有效范围、去向和转换规则，再讨论接口、页面、服务和存储如何围绕这些结构工作。
...
每一层都可以有自己的职责和结构，但不能再静默发明或越界复用“同一个业务对象”。
...
它要求明确结构的所有者和生命周期边界，以及主键、候选键、分区策略、字段注释、DTO 字段、前端查询模型、页面展示模型、表单模型、状态模型和跨层转换规则。
...
项目约定发现指南。它要求在真正设计或实现功能前，先读取目标项目的 `docs/.sdd/project-index.yml` 和 `docs/.sdd/project-conventions.md`，再结合邻近代码推断模块、包结构、数据结构所有权和生命周期边界、构建命令、API 包装、分页、错误处理、路由和持久化风格。
...
3. **明确所有权和生命周期**
   根据项目约定、已有设计和相邻代码中的一致模式，明确每个结构的产生点、有效范围、变化权限、转换终点和留存规则；模式冲突、有具体风险或缺少依据时交由设计者澄清。
...
- **Lifecycle-Bounded Structures**：每个参与数据流的结构都必须有明确的所有者和生命周期边界；未决边界会阻止设计定稿和实现。
